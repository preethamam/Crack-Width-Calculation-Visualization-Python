...
from crackprops.crackutils import CrackWidthUtils
...
class CrackAnalysis:
    ...
    def perform_skeletonization(self):
        ...
        elif self.method == "afmm":
            threshold = math.floor(max(self.binary_crack.shape[0], self.binary_crack.shape[1]) * self.prune_threshold)             
            skeleton, deltaU, dt = pyafmm.skeletonize(self.binary_crack, threshold, is_rgb=False)
            self.binary_skel = skeletonize(skeleton)
        else:
            raise ValueError(
